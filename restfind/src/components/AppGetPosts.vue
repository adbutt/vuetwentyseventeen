<template>
  <div v-if="isDataAvailable" class="container">
    <!-- Number of Posts -->
    <small v-if="filteredResults.length === wpPosts.length">{{ wpPosts.length }} Restaurants</small>
    <small v-else>Found {{ filteredResults.length }} of {{ wpPosts.length }} Restaurants</small>
...
export default {
  ...
  computed: {
    ...
    filteredResults() {
      if (this.wpPosts) {
        ...
        let filteredPosts = this.wpPosts.filter(post => {
          return (
            post.title.rendered.match(pattern) ||
            // post.vue_meta.custom_excerpt.match(pattern)
            post.vue_meta.pcodes.match(pattern)
            // post.vue_meta.postcodes.some(postc => postc.includes(adam))
          );
        });

        //further filter the results based on the delivery filters.
        // if (this.supFilters && this.supFilters.length) {
        //   filteredPosts = filteredPosts.filter(post =>
        //     post.vue_meta.supply_options.match(this.supFilters)
        //   );
        //   // return filteredPosts;
        // }
        if (this.supFilters && this.supFilters.length) {
          filteredPosts = filteredPosts.filter(post =>
            post.vue_meta.supply_options.some(supply_options =>
              this.supFilters.includes(supply_options.toLowerCase())
            )
          );
          // return filteredPosts;
        }
        ...
        if (this.appFilters && this.appFilters.length) {
          filteredPosts = filteredPosts.filter(post =>
            post.vue_meta.cuisines.some(cuisines =>
              this.appFilters.includes(cuisines.toLowerCase())
            )
          );
          return filteredPosts;
        } else {
  ...
  methods: {
    ...
    async getPosts(route = "restaurants", namespace = "wp/v2") {
      try {
        /* Note: the per_page argument is capped at 100 records by the REST API.
         * https://developer.wordpress.org/rest-api/using-the-rest-api/pagination/
         */

        const postsPerPage = 6; // default is 10.
